...
def maces_prepare_job_file(oModel_in):
    ...
    if iFlag_calibration == 1:
        ...
        sWorkspace_calibration_case = sWorkspace_calibration + slash + sCase
        if not os.path.exists(sWorkspace_calibration_case):
            os.mkdir(sWorkspace_calibration_case)
        else:
            pass
        sFilename_job = sWorkspace_calibration_case + slash + 'submit_maces.job'
        pass
    else:
