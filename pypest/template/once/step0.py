def pypest_prepare_pest_control_file(oPest_in, oModel_in):
    ...
    #pypest supports multiple tpl and ins files

    for i in np.arange(1,ntplfile+1):
        sIndex = "{:02d}".format(i)
        sLine1 = sWorkspace_pest_model + slash + 'template'+ sIndex + '.tpl'    
        sLine2 = 'hru.para\n'
        sLine = sLine1 + ' ' + sLine2
        ofs.write(sLine)
    ...
    for i in np.arange(1,ninsfile+1):
        sIndex = "{:02d}".format(i)
        sFilename_instruction = sWorkspace_pest_model + slash + 'instruction'+ sIndex + '.ins'    
        sFilename_output =  sWorkspace_pest_model + slash + 'instruction'+ sIndex + '.out'    
